...
# =================================================================
# 2. 유틸리티 함수 (데이터 0점 처리 및 이미지 인코딩 복원)
# =================================================================
def get_b64(path):
...
def clean_num(x):
    """영진님이 0으로 채우신 데이터를 안전하게 숫자로 변환하는 핵심 로직"""
...
if not df_vol.empty:
    ...
    if st.session_state.view == 'home':
        ...
        slides_html = "".join([f'<div class="slide"><img src="data:image/png;base64,{get_b64(os.path.join(LOGO_DIR, f))}"></div>' for n, f in LOGO_MAP.items() if get_b64(os.path.join(LOGO_DIR, f))])
    ...
    # =================================================================
    # 5. 상세 페이지 (음영 서식 완벽 복원)
    # =================================================================
    else:
        menu = st.session_state.sel_comp
        if menu in LOGO_MAP:
            b_logo = get_b64(os.path.join(LOGO_DIR, LOGO_MAP[menu]))
