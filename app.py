...
def fetch_data(sheet_name):
    """[수정] 영진님이 선택한 'XX월' 시트만 정확히 호출"""
    try:
        gsid = "14-mE7GtbShJqAHwiuBlZsVFFg8FKuy5tsrcX92ecToY"
        # 월 선택 시 '01월', '02월' 형태로 호출, 그 외(임시직)는 그대로 호출
...
with st.sidebar:
    ...
    # [수정] 월 선택을 먼저 배치하여 fetch_data에 전달
...
if not df_vol.empty:
    if st.session_state.view == 'home':
        ...
        if not df_temp.empty:
            ...
            for col in date_cols: t_final[col] = t_final[col].apply(clean_num)
