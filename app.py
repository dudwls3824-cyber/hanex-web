...
def clean_num(x):
    """영진님이 채운 0점을 포함하여 모든 숫자를 정확히 읽어냄"""
...
<style>
    ...
    .slider {{ background: white; height: 100px; margin-bottom: 30px; overflow: hidden; position: relative; border-radius: 12px; display: flex; align-items: center; }}
...
if not df_vol.empty:
    ...
    # --- 상세 페이지 (날짜별 열 출력 완전 복구) ---
    else:
        target = st.session_state.sel_comp
        if target in L_MAP:
            b_logo = get_b64(os.path.join(LOGO_DIR, L_MAP[target]))
        ...
        st.markdown(f"### 🏢 {target} 상세 현황 ({mon}월)")
        m_name = re.sub(r'\s+', '', target).upper()

        # 1. 물동량 상세 (날짜별 열 01, 02... 싹 다 복구)
        ...
        if not df_temp.empty:
            ...
            # 합계 행 추가
            t_total_row = ['일자별 합계', t_df['월 합계'].sum()] + t_df[date_columns].sum().tolist() if 'date_columns' not in locals() else ['일자별 합계', t_df['월 합계'].sum()] + t_df[date_cols].sum().tolist()
